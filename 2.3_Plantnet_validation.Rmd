...
## 2.3 Assign
```{r}
names <- read_csv("../Data/Names/Names_plants_plantnet.csv")
...
sum(!is.na(pol$plantnet_rank) & pol$plantnet_score>=0.60)

# relevant
interactions <- pol[(!is.na(pol$plant) | (!is.na(pol$plantnet_rank) & pol$plantnet_score >=0.60 ) & pol$plantnet_organ == "flower"),]
...
```{r}
# create network
network_interaction <- interactions |>  group_by(name, plant_taxa) |> summarize(total_interactions = n(), .groups = 'drop')
