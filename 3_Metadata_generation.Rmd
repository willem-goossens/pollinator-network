...
Load interactions
```{r}
interactions_obs <- read_csv("../Results/Interaction_dataset.csv", show_col_types = FALSE)
interactions_obs_safe <- interactions_obs
interactions_nat <-  read_csv("../Results/Interaction_dataset_nat.csv", show_col_types = FALSE)
interactions_nat_safe <- interactions_nat
```

```{r}
# get downsized databases with data present in both
interactions_obs <- interactions_obs |> dplyr::select(id, date, name, pol_species, pol_genus, pol_family, pol_rank, 
                                              plantnet_species, plant_taxa, plant_genus, plant_family, plant_rank_final,
                                              lat, lng, country)
interactions_obs$source <- "Observation.org"
...
# change name
colnames(interactions_nat) <- colnames(interactions_obs)

# save 
interactions_obs$id <- as.character(interactions_obs$id)
interactions_obs$date <- as.Date(interactions_obs$date, format = "%Y-%m-%d")
interactions_nat$date <- as.Date(interactions_nat$date, format = "%Y-%m-%d")

# combine
interactions <- bind_rows(interactions_obs, interactions_nat)
```
...
```{r}
# remove Georgia
nrow(interactions[interactions$euromed_region == "Georgia",])
interactions <- interactions[interactions$euromed_region != "Georgia",]
```
...
interactions <- interactions |> 
  mutate(time_period = cut(as.Date(date),
         breaks = as.Date(c("1995-01-01", "2010-01-01","2020-01-01" ,"2025-12-31")), right =F))
...
interactions_plot <- interactions |>   group_by(name, plant_taxa, eco_region) |>  filter(n() >= 2) |>
  ungroup()

p <- ggplot(interactions_plot) +
  ...
  scale_colour_manual(name="time_period", 
      values = c("1995-01-01" = "#1F78B4", "2010-01-01" = "#E31A1C", "2020-01-01" = "#33A02C"),
      labels = c(paste0("1995-2009 (n = ", period_counts[1], ")"),
                 paste0("2010-2020 (n = ", period_counts[2], ")"),
                 paste0("2020-2025 (n = ", period_counts[3], ")")))+
...
ggsave("../Images/timescale_period.png", p, width= 6, height= 3)
...
```{r}
# save
#write_csv(interactions, "../Results/Interaction_all.csv")

#interactions <- read_csv("../Results/Interaction_all.csv")

# remove data before 2010
interactions <- interactions[!interactions$date < "2010-01-01", ]
colnames(interactions)

# also assign to the other datasets
interactions_obs_safe$euromed_region <- interactions$euromed_region[match(interactions_obs_safe$id, interactions$id)]
interactions_obs_safe <- interactions_obs_safe[!is.na(interactions_obs_safe$euromed_region),]
interactions_obs_safe$eco_region <- interactions$eco_region[match(interactions_obs_safe$id, interactions$id)]
interactions_obs_safe$eco_biome <- interactions$eco_biome[match(interactions_obs_safe$id, interactions$id)]
interactions_obs_safe$eco_name <- interactions$eco_name[match(interactions_obs_safe$id, interactions$id)]

# assign to iNaturalist  
interactions_nat_safe$euromed_region <- interactions$euromed_region[match(interactions_nat_safe$observation_uuid, interactions$id)]
interactions_nat_safe$eco_region <- interactions$eco_region[match(interactions_nat_safe$observation_uuid, interactions$id)]
interactions_nat_safe$eco_biome <- interactions$eco_biome[match(interactions_nat_safe$observation_uuid, interactions$id)]
interactions_nat_safe$eco_name <- interactions$eco_name[match(interactions_nat_safe$observation_uuid, interactions$id)]

```


# 2 Additional
First create downsize dataset
```{r}
# create network
...
length(unique((interactions$plant_taxa[interactions$plant_rank_final=="Species"])))
length(unique(((interactions$pol_species))))

```


```{r}
subset <- interactions |> group_by(eco_name, country, name, plant_taxa) |> summarise(n= n())|>
  group_by(eco_name, country) |> summarise(n=n())
```
...
```{r}
# read data
cor2000 <-  rast("../Extra/CORINE/U2006_CLC2000_V2020_20u1.tif") |> crs(4326)
cor2006 <-  rast("../Extra/CORINE/U2012_CLC2006_V2020_20u1.tif") |> crs(4326)
cor2012 <-  rast("../Extra/CORINE/U2018_CLC2012_V2020_20u1.tif") |> crs(4326)
cor2018 <-  rast("../Extra/CORINE/U2018_CLC2018_V2020_20u1.tif") |> crs(4326)
...
# extract the values 
int_2000 <- terra::extract(cor2000, coord[,c(3)])
int_2006 <- terra::extract(cor2006, coord[,c(3)])
int_2012 <- terra::extract(cor2012, coord[,c(3)])
int_2018 <- terra::extract(cor2018, coord[,c(3)])
